...
\begin{document}
	...
	\begin{itemize}
		...
		% Nominal, categorical and ordinal features
	...
	\begin{table}[]
		...
		\begin{tabular}{ll|l}
			...
			\multirow{5}*{\textbf{Classifier}}     & Naïve Bayes             & GaussianNB, BernoulliNB, MultinomialNB                                                                                     \\ \cline{3-3}
			              & Decision Tree           & \begin{tabular}[c]{@{}l@{}}DecisionTree, ExtraTrees, RandomForest, \\ GradientBoosting\end{tabular}                        \\ \cline{3-3}
			              & Nearest Neighbor        & KNeighbors                                                                                                                 \\ \cline{3-3}
	...
	\begin{table}[]
		...
		\begin{tabular}{l|ll}
			...
			Ranking method    & \begin{tabular}[c]{@{}l@{}}The method used for ranking \\ the features (
				Subsection \ref{FSsubsec:FilterMethods})\end{tabular}               & \begin{tabular}[c]{@{}l@{}}T-test (SciPy)\\ Mutual Information (scikit-learn)\\ --\end{tabular}                                                                \\
	...
	% Introduction
	TPOT is an effective tool to find the best machine learning pipeline for a certain dataset (Subsection \ref{FSsubsec:AutomatedMachineLearning}). TPOT however has three restrictions on finding the correct pipeline when a high number of features is involved:
	...
	\begin{table}[]
		\caption{The experiment details for testing the non-trivial changes in TPOT. This experiment is rerun 5 times.}
		\label{tab:TPOTExpDetails}
		\begin{tabular}{l|l|l}
			\textbf{\begin{tabular}[c]{@{}l@{}}Experiment \\ factors\end{tabular}}                           & \textbf{Detailed values}                                                                                                                                                                    & \textbf{Remarks}                                                                                                                                                                                                                                    \\ \hline
			\textbf{Datasets}                                                                                & \begin{tabular}[c]{@{}l@{}}MicroOrganisms\\ Arcene\\ RSCTC\\ Psoriasis\end{tabular}                                                                                                         & \begin{tabular}[c]{@{}l@{}}High number of features\\ (Subsection \ref{FSsubsec:Datasets})\end{tabular}                                                                                                                                                                  \\ \hline
			\textbf{\begin{tabular}[c]{@{}l@{}}Performance \\ measurement\end{tabular}}                      & \begin{tabular}[c]{@{}l@{}}One type: \textit{FS\_accuracy}\\ - $\beta = 0.99$\end{tabular}                                                                                                             & \begin{tabular}[c]{@{}l@{}}Addition of correction factor\\ (Equation \ref{eq:FSAccuracy})\end{tabular}                                                                                                                                                              \\ \hline
			\textbf{\begin{tabular}[c]{@{}l@{}}TPOT input \\ parameters\end{tabular}}                        & \begin{tabular}[c]{@{}l@{}}One set of input values:\\ - max. optimization time = 120 min\\ - max. alg. evaluation time = 10 min\\ - pop. size = 20 (twice reduced to 12)\\ - train size = 0.9\end{tabular} & \begin{tabular}[c]{@{}l@{}}Explanation of input values:\\ - The time one run should take (2 hours)\\ - The evaluation time of one pipeline \\ - The number of pipelines in one generation\\ - The number of samples used for training\end{tabular} \\ \hline
	...
	\begin{table}[]
		\caption{The performance of the final pipeline for the different types of TPOT and the mentioned dataset. The five reruns are averaged into this one result.}
		...
		\begin{tabular}{ll|ll|ll}
			...
			\multirow{4}{*}{\textbf{Datasets}}  & MicroOrganisms & 0.51                                                        & 0.63                                                                  & 0.56               & 0.57                                                                   \\
			& Arcene         & 0.37                                                        & 0.70                                                                  & 0.81               & 0.66                                                                   \\
			& RSCTC          & 0.00                                                       & 0.19                                                                 & 0.00              & 0.47                                                                       \\
			& Psoriasis      & 0.01                                                       & 0.12                                                                 & 0.03                   & 0.50                                                                      
		\end{tabular}
	\end{table}
	...
	%After plotting the p-values for the distributions (Figures \ref{fig:PMeanFits}, \ref{fig:PVarianceFits}, \ref{fig:PMeanFitsACA} and \ref{fig:PVarianceFitsACA}), the kNN imputation with $k=1$ seems to the least biased results in both mean and variance.
	...
	\begin{table}[H]
		...
		\begin{tabular}{l|l}
			...
			K-means                                                                & \begin{tabular}[c]{@{}l@{}}Assigns genes to clusters closest to cluster centers\\ Changes cluster centers after new genes are assigned\\ \\ Tested number of clusters: 5, 8, 9 10, 11, 12, 15, 20.\end{tabular} \\ \hline
