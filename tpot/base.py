...
from .gp_types import Output_Array, Input_Array
from .gp_deap import eaMuPlusLambda, mutNodeReplacement, _wrapped_cross_val_score, cxOnePoint

# NEW IMPORTS
from.config.feature_selection import fs_config, order_config, fs_pipeline_steps
...
class TPOTBase(BaseEstimator):
    ...
    def __init__(self, generations=100, population_size=100, offspring_size=None,
                 ...
                 verbosity=0, disable_update_check=False, feature_selection=False, fs_modifier=None):
    ...
    def _setup_pset(self):
        ...
        if self.feature_selection:
            self._pset = gp.PrimitiveSetTyped('MAIN', [Input_Array], Output_Array)
        else:
    ...
    def _add_operators(self):
        for operator in self.operators:
            ...
            # START ADDITIONAL FS code
                ...
                # Add for every feature selection algorithm a start with input array, so primitives will start with it
                p_types = (input_types, Input_Array)
                self._pset.addPrimitive(operator, *p_types)
            # END ADDITIONAL FS code
    ...
    def _setup_toolbox(self):
        ...
        # START ADDITIONAL FS code: max pipeline length is reduced by 1 to not have lengths of 4
        self._toolbox.register('expr', self._gen_grow_safe, pset=self._pset, min_=1, max_=self.max_pipeline_length)
    ...
    def _evaluate_individuals(self, individuals, features, target, sample_weight=None, groups=None):
        ...
        partial_wrapped_cross_val_score = partial(
            ...
            feature_selectors=fs_pipeline_steps,
            fs_modifier = self.fs_modifier
        )

        result_score_list = []
        # Don't use parallelization if n_jobs==1
        if self.n_jobs == 1:
            for (sklearn_pipeline, ind_str) in zip(sklearn_pipeline_list, eval_individuals_str):

                # print pipeline
                print('starting evaluation of', ind_str)
    ...
    def _gen_grow_safe(self, pset, min_, max_, type_=None):
        ...
        def condition(height, depth, type_, feature_selection):
            """Stop when the depth is equal to height or when a node should be a terminal."""

            # START ADDITIONAL FS code
            if feature_selection:
                # Does not add sequence method when type is not suitable, max height is reached.
                # Flips a coin to add final feature selection method when nd_array is found
                # (Randomness may be removed, depends on author's wishes)
                return type_ not in [np.ndarray, Input_Array, Output_Array] or depth == height or \
                       (random.getrandbits(1) and type_ == np.ndarray)
            # END ADDITIONAL FS code
    ...
    def _generate(self, pset, min_, max_, condition, type_=None):
        ...
        # START ADDITIONAL FS code, add possibility of order
        if self.feature_selection:
            add_order = random.getrandbits(1)

            order_prims = []
            for prim in pset.primitives[Input_Array]:
                order_prims.append(prim)
        ...
        while len(stack) != 0:
            ...
            if condition(height, depth, type_, self.feature_selection):

                # START ADDITIONAL FS code
                if self.feature_selection:
                    # Checks if a final feature selection array should be added
                    if type_ == np.ndarray:
                        fs_prims = []
                        # Retrieve all feature selection primitives
                        for prim in pset.primitives[type_]:
                            if prim.args[0] == Input_Array:
                                fs_prims.append(prim)
                        # Choose a random feature selection primitive
                        prim = np.random.choice(fs_prims)
                        expr.append(prim)
                        # Add arguments to stack for terminals to be added
                        for arg in reversed(prim.args):
                            stack.append((depth, arg))
                        continue

                    elif type_ == Input_Array and add_order:
                        add_order = False
                        order_prims = []
                        # Retrieve all order primitives
                        for prim in pset.primitives[Input_Array]:
                            order_prims.append(prim)
                        ...
                        # Choose a random feature selection primitive
                        prim = np.random.choice(order_prims)
                        expr.append(prim)
                        # Add arguments to stack for terminals to be added
                        for arg in reversed(prim.args):
                            stack.append((depth, arg))
