...
# Import needed for splitting feature selection pipeline
...
def _wrapped_cross_val_score(sklearn_pipeline, features, target,
                             cv, scoring_function, sample_weight=None, groups=None, feature_selectors=None,
                             fs_modifier=None):
    ...
    if (not fs_modifier is None):
        ...
        for i in reversed(range(len(sklearn_pipeline.steps))):
            ...
            if (sklearn_pipeline.steps[i][0]) in feature_selectors:
                  fs_pipeline = Pipeline(steps=sklearn_pipeline.steps[:i+1])
                  break

        # If part of it is a feature selection pipeline:
        if not fs_pipeline is None:
            n_features = fs_pipeline.fit_transform(features, target).shape[1]
        else:
    ...
    try:
        with warnings.catch_warnings():
            ...
            if not fs_modifier is None:
                scores = [[unlisted_score - fs_modifier * n_features for unlisted_score in score] for score in scores]
                print("The final score becomes %f, after subtracting it by %i features with modifier %f"
                      % (np.average(np.asarray(scores)), n_features, fs_modifier))
            # END ADDITIONAL FS code
