...
sample_values, skin_type_values, gene_ids, sample_ids, series = SDE.extract_data(data_names[0])
...
sign_group_1, sign_group_2, sign_gene_ids = SE.extract_significant_genes(sample_values, skin_type_values, gene_ids,
                                                                         target_groups=[0, 1], threshold=0.001,
...
final_values, final_gene_ids = RT.remove_double_genes(np.concatenate(sign_group_1b, sign_group_2b),
                                                      sign_gene_ids_b, sign_gene_ids)

# Find processes with similar relation
print("Searching for process relations between genes...")
sign_process, = RT.testing_gene_relations(final_gene_ids, gene_set, printing=True)
