...
\begin{document}
	...
	% The second discussed methods are about feature extraction, a technique that creates a new smaller set of features derived from the old feature space.
	
	% Regression: https://www.sciencedirect.com/science/article/pii/S0169743905000031
	% Ridge regression: https://amstat.tandfonline.com/doi/abs/10.1080/00401706.2000.10485983?src=recsys#aHR0cHM6Ly9hbXN0YXQudGFuZGZvbmxpbmUuY29tL2RvaS9wZGYvMTAuMTA4MC8wMDQwMTcwNi4yMDAwLjEwNDg1OTgzP25lZWRBY2Nlc3M9dHJ1ZUBAQDA=
	...
	A disadvantage of statistics is that it is mainly focused on binary testing of and not on testing of multiple groups. Several options are available for a multiple group testing, also called analysis of variance (ANOVA). ANOVA needs equal group sizes, otherwise the results of the tests are less reliable. The less powerful Kruskall Wallis test can be used if this reliability is needed. Another disadvantage is that the t-test assumes the groups follow a certain distribution, which might not be the case\cite{heiberger2004statistical}.
	...
	\begin{table}[]
	\centering
	\caption{The four variables with their possible values in the first experiment.}
	...
	\begin{table}[]
		...
		\begin{tabular}{l|ll}
			...
			\multirow{2}{*}{Filter methods}   & \multirow{2}{*}{\begin{tabular}[c]{@{}l@{}}Basic Filter Methods\\ (Algorithm \ref{alg:FilterAlgorithm})\end{tabular}} & \begin{tabular}[c]{@{}l@{}}- T-test\\ - Thresholds: 0.05, 0.01\\ --\end{tabular}                                                                                                             \\
			&                                                                                              & \begin{tabular}[c]{@{}l@{}}- Mutual Information\\ - Thresholds: 0.1, 0.2\end{tabular}                                                                                                        \\ \hline
			\multirow{5}{*}{Wrapper methods}  & \begin{tabular}[c]{@{}l@{}}Forward selection\\ (Algorithm \ref{alg:ForwardSelection})\end{tabular}                     & \begin{tabular}[c]{@{}l@{}}- Order: Random, Mutual Information\\ - Evaluation function: Naive Bayes\\ - Alpha: 0.01, 0.001\\ --\end{tabular}                                                 \\
			...
			& \begin{tabular}[c]{@{}l@{}}PTA\\ (Algorithm \ref{alg:PTA})\end{tabular}                                   & \begin{tabular}[c]{@{}l@{}}- Order: Random, Mutual Information\\ - Evaluation function: Naive Bayes\\ - {[}l, r{]} = {[}20, 10{]}, {[}5, 2{]}\\ - Alpha: 0.01, 0.001\\ --\end{tabular} \\
			& \begin{tabular}[c]{@{}l@{}}Floating search\\ (Algorithm \ref{alg:FloatingSearch})\end{tabular}                       & \begin{tabular}[c]{@{}l@{}}- Order: Random\\ - Evaluation function: Naive Bayes\\ - Alpha: 0.01\end{tabular}                                                                                 \\
			& \begin{tabular}[c]{@{}l@{}}Simulated Annealing\\ (Algorithm \ref{alg:SASearch})\end{tabular}                   & - \textit{Testing needs to occur to find good parameter values}                                                                                                                                                                                           \\ \hline
			\multirow{2}{*}{Embedded methods} & \begin{tabular}[c]{@{}l@{}}Forward selection\\ (Algorithm \ref{alg:EmbeddedForwardSelectionAlgorithm})\end{tabular}                     & - \textit{Testing needs to occur to find good parameter values}                                                                                                                                                                                            \\
			& \begin{tabular}[c]{@{}l@{}}Backward elimination\\ (Algorithm \ref{alg:EmbeddedBackwardEliminationAlgorithm})\end{tabular}                  & - \textit{Testing needs to occur to find good parameter values}                                                                                                                                                                                           
		\end{tabular}
	\end{table}
	
	% \subsection{Feature Extraction Algorithms}
	% \label{subsec:FeatureExtractionAlgorithmsEvaluation}
	...
	%\section{Feature Selection Evaluation Spectra}
	%\label{app:FeatureSelectionEvaluationSpectra}
